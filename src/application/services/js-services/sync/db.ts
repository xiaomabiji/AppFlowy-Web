...
export class Db {
  ...
  private _storeTask: Promise<number>|null
  ...
  async setLastMessageId(value: string) {
    const tx = this.db.transaction('meta', 'readwrite');
    const meta = tx.objectStore('meta');

    await meta.put(value, 'lastMessageId');
  }
  ...
  async mount(doc: Y.Doc) {
    ...
    doc.on('update', (bytes: Uint8Array, lastMessageId: string) => {
      ...
      if (this._storeTask) {
        this._storeTask = this._storeTask.then(() => {
          return this.storeUpdate(update)
        });
      } else {
        this._storeTask = this.storeUpdate(update);
      }
    })
  ...
  async storeUpdate(update: Update): Promise<number> {
    ...
    if (update.lastMessageId && update.lastMessageId > this.lastMessageId) {
      await this.setLastMessageId(update.lastMessageId);
    }
  ...
  async deleteDoc(docId: string) {
    ...
    const updates = tx.objectStore('updates')
    const updateIndex = updates.index('ixDocId')
    const indexKeys = await updateIndex.getAllKeys(docId)
